...
预算金额单位为“元”。例如填写 `500000.00` 表示人民币 50 万元。
...
2. 进入“项目申报”。
3. 点击“新建项目”。
4. 选择申报批次、项目类型、项目类别，填写摘要和预算金额（元）。
5. 点击“保存草稿”。
...
3. 停留在“待审核”页签。
4. 找到当前阶段为“专家评审”的项目。
5. 操作列有两个圆形按钮：眼睛图标是详情，蓝色勾选图标是“审核处理”。
6. 点击蓝色勾选按钮，弹出“审核处理”。
...
| 预算金额单位不清楚 | 页面只显示“预算金额” | 已标明“预算金额（元）”，详情和导出也同步 |
| 账号/消息列表不清楚顺序 | 无序号，默认排序语义不明显 | 已增加序号，默认按时间从早到晚 |
| 专家找不到评分入口 | 评分入口在“审核任务”蓝色勾选按钮，不在全周期 | 已补截图和说明 |
