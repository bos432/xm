param(
    [string]$ReportPath = (Join-Path $PSScriptRoot "legacy-migration-go-live-drill-operator-pack-validation.json"),
    [string]$OperatorPackPath = (Join-Path $PSScriptRoot "legacy-migration-go-live-drill-operator-pack.json")
)
...
$validStatuses = @('ready', 'not_ready', 'blocked', 'missing', 'open', 'warning', 'pass')

if (-not $operatorPack) {
    Add-Issue $issues 'blocker' 'operator_pack' 'missing_operator_pack' 'go-live drill operator pack report is missing.'
} else {
    ...
    if ($steps.Count -ne 9) {
        Add-Issue $issues 'blocker' 'steps' 'step_count_mismatch' "operator pack should contain 9 drill steps, found $($steps.Count)."
    }

    if ($files.Count -ne 13) {
        Add-Issue $issues 'blocker' 'files' 'file_count_mismatch' "operator pack should contain 13 file entries, found $($files.Count)."
    }
    ...
    foreach ($step in $steps) {
        ...
        if (Test-Blank $step.owner) { Add-Issue $issues 'warning' 'steps.owner' 'blank_step_owner' "step $($step.order) owner is blank." }
    ...
    $readyStepCount = @($steps | Where-Object { $_.status -eq 'ready' -or $_.status -eq 'pass' }).Count
    $pendingStepCount = @($steps | Where-Object { $_.status -eq 'not_ready' -or $_.status -eq 'open' -or $_.status -eq 'warning' }).Count
    $blockedStepCount = @($steps | Where-Object { $_.status -eq 'blocked' -or $_.status -eq 'missing' }).Count
    ...
    $firstOpenStep = @($steps | Where-Object { $_.status -ne 'ready' -and $_.status -ne 'pass' } | Select-Object -First 1)
...
$report = [ordered]@{
    ...
    mode = 'preview_only'
    note = 'This report validates the go-live drill operator pack structure and internal counts. It does not copy files, import records, switch traffic, update templates, or write database records.'
...
Write-Host "Legacy migration go-live drill operator pack validation written to $ReportPath"
